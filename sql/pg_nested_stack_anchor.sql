...
--
-- Note the direction: the review predicted the guard would trip *later* than
-- intended.  It trips *earlier* -- the _PG_init anchor is the shallowest point in
-- the backend, so at any nested depth QuickJS believes it is nearer the end of the
-- stack than it is.  Same root cause, opposite symptom, and the symptom is what
-- makes it a functional bug rather than only a weakened guard.
--
