...
-- 5) an array return type: null/undefined is SQL NULL like any other type (the
-- full matrix lives in pg_array_null_returns.sql), while a non-null value of
-- the wrong shape must still raise rather than silently becoming NULL.
CREATE FUNCTION nrp_arr_null() RETURNS int[] LANGUAGE pljs AS $$ return null; $$;
SELECT nrp_arr_null() IS NULL AS arr_null_is_sqlnull;
CREATE FUNCTION nrp_arr_bad() RETURNS int[] LANGUAGE pljs AS $$ return 5; $$;
SELECT nrp_arr_bad();
