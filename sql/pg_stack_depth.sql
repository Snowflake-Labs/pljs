...
-- the bound observable -- otherwise the test passes either way and this commit has
-- no regression coverage".  A previous version of this comment claimed it could not
-- be done, on the grounds that the budget is derived once in _PG_init so a SET has
-- no effect.  That was wrong.  _PG_init runs when the library is first loaded *in a
-- given backend*, which is the first pljs call in that session -- so a SET that
-- happens before any pljs function is called is read.  \c gives a fresh backend per
-- setting, which is how the section below compares two of them.
--
