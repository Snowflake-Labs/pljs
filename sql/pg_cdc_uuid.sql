...
-- comparison against a uuid column, and the bare-uuid result behaviour
...
DO $$
  ...
  // Pin the bare-uuid result behaviour alongside the ::text form callers use.
  // Both now agree: the datum->JS conversion fallback routes an otherwise
  // unhandled type through its output function, so a bare uuid round-trips
  // instead of yielding its raw 16 bytes decoded as UTF-8.
