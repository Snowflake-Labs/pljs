...
-- because the CDC adapter keeps LSNs as BigInt and must never accidentally add
-- a Number to one.
DO $$
  try {
    const r = 1n + 2;
    pljs.elog(NOTICE, 'mix BigInt+Number: no throw, r=' + r);
  } catch (e) {
    pljs.elog(NOTICE, 'mix BigInt+Number throws: ' + e.name);
  }
  pljs.elog(NOTICE, 'pure BigInt: 2n**10n=' + (2n ** 10n) + ' cmp=' + (9007199254740993n > 9007199254740992n));
$$ LANGUAGE pljs;
