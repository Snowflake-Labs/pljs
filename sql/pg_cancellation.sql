...
-- A try/catch around the runaway loop must NOT be able to swallow the
-- cancellation.  Without this the whole commit is, as the review put it, defeated
-- by four characters of user JS.
--
-- It holds because the vendored QuickJS marks the interrupt error uncatchable:
-- deps/quickjs/quickjs.c raises JS_ThrowInternalError(ctx, "interrupted") and then
-- calls JS_SetUncatchableError(ctx, ctx->rt->current_exception, TRUE), so
-- JS_CallInternal will not hand it to a catch block.  Checked in the vendored
-- source rather than assumed of QuickJS in general.
SET statement_timeout = '400ms';
...
DO $$
  try {
    ...
    // Must be unreachable: if the interrupt were catchable this would swallow it
    // and the DO block would return normally instead of being cancelled.
    pljs.elog(NOTICE, 'SWALLOWED THE CANCELLATION: ' + e.message);
  }
$$ LANGUAGE pljs;
...
SELECT 1 AS alive_after_uncatchable;

-- Same again with a finally block, which runs on the way out and must not be able
-- to restart the loop or suppress the error either.
SET statement_timeout = '400ms';
...
DO $$
  try {
    ...
    pljs.elog(NOTICE, 'finally ran');
  }
$$ LANGUAGE pljs;

RESET statement_timeout;

SELECT 1 AS alive_after_finally;
