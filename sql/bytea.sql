...
DO $$
  ...
  // A bytea result is a Uint8Array of the exact stored bytes, not a string.
  // Decoding the bytes as UTF-8 into a JS string would replace every byte that
  // is not valid UTF-8 with U+FFFD and lose it, so compare the bytes -- which is
  // what this round-trip is really asserting.
  const decoded = String.fromCharCode.apply(null, result);
